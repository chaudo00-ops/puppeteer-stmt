...
export class BillingPdfEditor extends PdfEditor<BillingPDFContext> {
	...
	constructor(statementTitle: string, statement_details: TBillingStatementDetails_Display) {
		super(statementTitle); // Call the parent constructor
	...
	drawBillTo() {
		const payment_profile = this.statement_details.payment_profile;
		// Move to next line
	...
	drawStatementDetails() {
		...
		this.drawLeftAlignedText("Details", LEFT_MARGIN, this.interSemiBold, FONT_H3);
		...
		const statementDetailsLines = [
			{
				label: "Account name",
				value: account.ads_sub_acc_name,
			},
			{
				label: "Account ID",
		...
		statementDetailsLines.forEach(({ label, value }) => {
			...
			const textLength = this.interRegular.widthOfTextAtSize(`${value}`, FONT_REG);
			...
			if (!overflowText) {
				...
				this.drawDashedLine(dashStart, dashEnd, FONT_SM, this.interRegular);
			} else {
		...
		this.drawLeftAlignedText(
			`Summary for ${account_balance.billing_period_start} \u2013 ${account_balance.billing_period_end}`,
			leftBoundary,
			this.interSemiBold,
			FONT_H3,
		);
		...
		const statementSummaryLines = [
...

			{
				...
				value: account_balance.total_ad_spend_adjusted,
			},
			{
	...
	drawActivityTable() {
		...
		const labelsKey_txn: readonly (keyof Pick<
			TFields_v2_monthly_campaign_spend_ui,
			"cpgn_name" | "imp" | "cost"
		>)[] = ["cpgn_name", "imp", "cost"] as const;
		const labelsName = ["Description", "Impressions", "Amount"] as const;

		const balanceAdjustmentTable = this.statement_details.balance_adjustments;
		const labelsKey_balAdj: readonly (keyof Pick<
			TFields_v2_balance_adjustments,
			"bal_adj_id" | "notes" | "applied_amount"
		>)[] = ["bal_adj_id", "notes", "applied_amount"] as const;

		// Table settings
		/** Table Header */
		let isHeader = true;

		/* Campaign spends details */
		...
		transactionTable.forEach((row, rowIndex) => {
			...
			const rowHeight = this.predictRowHeight(row, labelsKey_txn[0]);
			...
			if (isHeader) {
				...
				this.documentPage.drawText("Activity Details", {
					x: LEFT_MARGIN,
					y: this.context.Y,
					size: FONT_H3,
					font: this.interBold,
			...
			// Draw cell text
			this.drawTableRow(row, rowIndex, labelsKey_txn, "activity");
		});

		/* Balance adjustments details */

		/** 0 or 1: helps determine the correct alternating color of the first balance adjustment record */
		const shift = transactionTable.length % 2;
		balanceAdjustmentTable.forEach((row, rowIndex) => {
			// Predict the row height in the soon to be drawn table row
			const rowHeight = this.predictRowHeight(row, labelsKey_balAdj[0]);
			...
			// Draw cell text
			this.drawTableRow(row, rowIndex + shift, labelsKey_balAdj, "activity");
		});
	...
	drawPaymentsReceived() {
		...
		const labelsName = ["Date", "Description", "Amount"] as const;
		...
		if (paymentTable.length === 0) {
			...
			this.documentPage.drawText("Payments Received", {
				x: LEFT_MARGIN,
				y: this.context.Y,
				size: FONT_H3,
	...
	drawTotalSpendingBalance() {
		...
		let labelWidth = this.interRegular.widthOfTextAtSize("Subtotal :", FONT_REG);
		...
		labelWidth = this.interRegular.widthOfTextAtSize("Total :", FONT_REG);
	...
	drawTotalPaymentsReceived() {
		...
		let labelWidth = this.interRegular.widthOfTextAtSize("Tax :", FONT_REG);
		...
		labelWidth = this.interRegular.widthOfTextAtSize("Total payments received :", FONT_REG);
		this.documentPage.drawText("Total payments received :", {
			x: LEFT_MARGIN + COL_WIDTH_LG + COL_WIDTH_SM - labelWidth,
			y: this.context.Y,
			size: FONT_REG,
			font: this.interSemiBold,
		...
		lines.forEach(line => {
			...
			switch (alignment) {
				...
				default:
					...
					throw getResultId(
						"internalErr",
						`(EFA203) invalid alignment ${_exhaustiveCheck}`,
					);
			}
		});
