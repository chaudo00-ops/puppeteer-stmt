...
export async function collectStatementDetails(
	...
	const payment_profiles: TFields_v2_payment_profiles[] = await Promise.all(
	...
	const monthly_account_balances: TBillingStatementDetails["monthly_account_balance"][] =
		await Promise.all(
			pmt_prf_link_history.map(pair =>
				fetchAccountBalance({
					sub_acc_id,
					pmt_prf_id: pair.linked_pmt_prf_id!,
					pmt_sub_acc_id: pair.linked_pmt_sub_acc_id!,
					start_month,
				}),
			),
		);
	...
	const res: TBillingStatementDetails[] = payment_profiles.map((payment_profile, index) => ({
		account,
		payment_profile: payment_profiles[index],
